...
class GameTestClient:
    ...
    def start_game(self, room_name):
        return self.test_client.post(
            f"/game/{GameEndpoints.START_GAME}?room_name={room_name}",
            headers=self.header,
        )

    def new_game(self, room_name):
        return self.test_client.post(
            f"/game/{GameEndpoints.NEW}?room_name={room_name}", headers=self.header
        )

    def player_join(self, room_name):
        return self.test_client.post(
            f"/game/{GameEndpoints.JOIN}?room_name={room_name}", headers=self.header
        )
...
def basic_game_setup_with_3_players(
    ...
    def _game_and_clients():
        ...
        client3 = GameTestClient(valid_user_token("user3"), test_client)
        room_name = "test_room"
        ...
        assert client3.player_join(room_name).status_code == status.HTTP_200_OK
...
def test_game(basic_game_setup_with_3_players):
